...
numChannels = 19;
fs = 512;
condition = input("Choose one condition: threat, sound, rock: ",'s');

%% ECG Feature Extraction

for part = 1:numPartic
    load('/Users/davidteixeira/Documents/GitHub/DavidJMT/EEG_Processed/EEG_'+string(participants(part))+'_processed.mat','fs','ECG_filt') 
    QRS = ecg_processing(ECG_filt,1,fs);
end

%% Feature Extration

stimulus_length = (round(66*fs)): (round(67*fs)); %has to be 1s otherwise it doesnt work?
neutral_length = (15*fs):fs:(55*fs);

%stimulusData and neutralData will be posteriorly used in the connectivity analysis
stimulusData = zeros(numChannels,length(stimulus_length),numPartic);
neutralData = zeros(numChannels,length(stimulus_length), length(neutral_length),numPartic);
...
for part = 1:numPartic
    ...
    for c=1:length(channels)-2 %for all the channels
        ...
        if strcmp(condition,'threat')
            [~,condit_idx]=min(abs(t-66.5)); %threat -> 66.5s (threat_idx -> [66 66.5]s)
...
fprintf("\n\nRelevant features based on Kruscal-Wallis\n\n")
...
%%
% Initialize an array to store the p-values and corresponding feature labels
p_values = [];
feature_labels = {};
...
for i = 1:length(channels) - 2
    for j = 1:length(bands)
        ...
        [p, tbl, stats] = kruskalwallis([all_subj_feats(:, 1, find(columnNames == label)), ...
                                         all_subj_feats(:, 2, find(columnNames == label)), ...
                                         all_subj_feats(:, 3, find(columnNames == label)), ...
                                         all_subj_feats(:, 4, find(columnNames == label))], ...
                                        {'t(neutral)', 't(stimulus)', 't(stimulus+1)', 't(stimulus+2)'}, 'off');
        
        % Store the p-value and feature label
        p_values = [p_values; p];
        feature_labels = [feature_labels; label];
    end
end

% Combine p-values and labels into a table for sorting
results_table = table(p_values, feature_labels, 'VariableNames', {'PValue', 'FeatureLabel'});

% Sort the table by p-value in ascending order
sorted_results = sortrows(results_table, 'PValue');

% Select the top 5 features
top_features = sorted_results(1:5, :);

% Display the top 5 features
disp('Top 5 Features based on Kruskal-Wallis test (even if p > 0.05):');
for k = 1:height(top_features)
    fprintf('Feature: %s, p-value = %.4f\n', top_features.FeatureLabel{k}, top_features.PValue(k));
end

%% Generate scores and labels for ROC analysis

% Initialize variables for storing AUC values and corresponding feature indices
numFeatures = size(all_subj, 3);
auc_values = zeros(1, numFeatures);
feature_indices = 1:numFeatures;

% Generate labels for ROC analysis
labels = [ones(numPartic, 1); zeros(numPartic * 3, 1)]; % Adjust labels size
...
for i = 1:length(channels) - 2
    for j = 1:length(bands)
        ...
        % Extract feature scores for each participant and each segment
        feature_scores = reshape(all_subj(:, :, (i - 1) * length(bands) + j), [], 1);

        % Calculate ROC curve and AUC
        [X, Y, T, AUC] = perfcurve(labels, feature_scores, 1);

        % Store AUC value
        auc_values((i - 1) * length(bands) + j) = AUC;
    end
end

% Sort features based on AUC values (descending order)
[sorted_auc_values, sorted_indices] = sort(auc_values, 'descend');

% Display top 5 features with channel and band information
top_feature_indices = sorted_indices(1:5);
top_feature_auc = sorted_auc_values(1:5);

disp('Top 5 Features:');
for k = 1:numel(top_feature_indices)
    idx = top_feature_indices(k);
    fprintf(' %d. %s, AUC = %.4f\n', k, columnNames{idx}, top_feature_auc(k));
end
...
for part=1:numPartic
    load('/Users/davidteixeira/Documents/GitHub/DavidJMT/DataDTF\dtf'+string(part)+'.mat','DTF') %load dtf file obtained from eConnectome
